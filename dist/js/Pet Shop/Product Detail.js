...
function insertProductData(rootEle, data) {
    ...
    $(thumbnailImage).css({ "max-height": "640px" });
    let imagesContainerEle = $(rootEle).find(".ezMall-stick-slide")[0];
    imageArr.forEach((item, index) => {
        let imageItem = `
        <button id="img-${index}" class="" onclick="changeThumnail('img-${index}')" style = "padding:0px; border:0px" >  
...
function ProductDetailGenerateCodeStart() {
    $("div[ez-mall-type='productDetail']").each(function (i) {
        ProductDetailGenerateCodeItem(this)
    });
}
...
function VariantCheck(variantId, optionId) {
    ...
    for (let i = 0; i < indexOption; i++) {
        ...
        if (checkedInput != null && checkedInput.length > 0) {
            variantValid = variantValid.filter(variant => variant.option_value.filter(option => option.id == $(checkedInput)[0].id).length > 0)
        }
    }
    ...
    for (let i = indexOption + 1; i < option.length; i++) {
        ...
        checkedInput = $(`#${option[i].id} input[type=radio]:checked`);
    ...
    if (variantValid != null && variantValid.length == 1) {
        ...
        if ($(`.ezMall-quantity-input`).val > productData.inventory) {
            $(`.ezMall-quantity-input`).val(productData.inventory)
        }
    }
    if (indexOption + 1 < option.length) {
        let nextCheckedOption = $(`#${option[indexOption + 1].id} input[type=radio]:checked`);
        if (nextCheckedOption != null & nextCheckedOption.length > 0) {
            VariantCheck(option[indexOption + 1].id, $(nextCheckedOption)[0].id);
        }
    }
}
...
function addToCart(isBuyNow = false) {
    ...
    if (is_variant) {
        options.forEach(option => {
            ...
            if (optionChecked.length != 0) {
                arrayOption.push(optionChecked[0].id)
            } else {
                arrayAlert.push(option.name);
            }
        })
    }
    ...
    let joinString = arrayOption.join(",");
